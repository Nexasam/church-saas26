...
class DummyDataSeeder extends Seeder
{
    ...
    private function createMemberOnly($church, $department, array $usedEmails): void
    {
        ...
        // Seed 20 plain members with no department
        $this->command->info('Seeding plain members (no department)...');
        for ($i = 0; $i < 20; $i++) {
            [$first, $last] = $this->randomName();
            $member = Member::create([
                'first_name' => $first,
            ...
            $church->members()->syncWithoutDetaching([
                $member->id => [
                    ...
                    'is_active'       => true,
                    'joined_at'       => now()->subDays(rand(30, 730)),
                ]
            ]);
        }
        $this->command->info('Created 20 plain members.');
    }
    ...
    private function createMemberAndUser($church, $department, $role, $password)
    {
        ...
        $email = strtolower(
            preg_replace('/[^a-z.@]/i', '', "{$firstName}.{$lastName}@grace.org")
        );
